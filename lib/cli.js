...
e.on('list new users',function(str){
  cli.responders.listNewUsers(str);
});
...
cli.responders.help = function(){
  ...
  var commands = {
    ...
    'More user info --{user email}' : 'Show details of a specified user',
    'list new users' : 'Show all the users who have signed up in the last 24 hours',
    'list menus' : 'Show all current menu items',
    'list new orders' : 'Show all the order who have placed in 24 hours',
    'More order info --{order id}' : 'Show details of a specified order',
  };
...
cli.responders.listNewUsers = function(){
  _data.list('users',function(err, userIds){
    if(!err && userIds){
      cli.verticalSpace();
      // Create instance of last 24 hours
      var yesterday = Math.round(new Date().getTime() / 1000) - (24 * 3600);
      userIds.forEach(function(userId){
        _data.read('users',userId,function(err,userData){
          if(!err && userData){
            // Compare time that user signuped and 24 hours instance
            if(userData.createdTime > yesterday){
              console.log("Name: "+userData.firstname+' '+userData.lastname);
              console.log('Email: '+userData.email);
              cli.verticalSpace();
            }
    ...
    if(!err && orderIds){
      ...
      orderIds.forEach(function(orderId){
        _data.read('orders',orderId,function(err,orderData){
          if(!err && orderData){
            if(orderData.createdTime > yesterday){
              // Compare time that order placed and 24 hours instance
              console.log("OrderId: "+orderId);
              console.log("Create Time: "+orderData.createdTime);
              cli.verticalSpace();
            }
          }
        })
  ...
  if(orderId){
    ...
    _data.read('orders',orderId,function(err,orderData){
      if(!err && orderData){
        ...
        cli.verticalSpace();
        console.log('not found this order id');
      }
    })
  } else {
    cli.verticalSpace();
